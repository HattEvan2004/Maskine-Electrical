...
export default function Header({ onNavigate, activeSection }: HeaderProps) {
  ...
  const phones = site?.phones?.length ? site.phones : [
    { label: 'Anthony', number: '902-802-5306' },
    { label: 'Mich', number: '902-830-5921' },
  ];
    ...
    <header className="w-full z-50 transition-all duration-300">
      <div className="bg-amber-500 text-slate-950 font-sans text-xs sm:text-sm py-2 px-4 flex flex-wrap justify-between items-center gap-2 font-semibold border-b border-amber-400 relative z-50">
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-1">
          <a href={`tel:${phones[0].number}`} className="flex items-center gap-1.5 hover:underline text-slate-950">
            <Phone size={13} className="text-slate-950 font-bold" />
            <span>{phones[0].label}: <strong className="font-bold">{phones[0].number}</strong></span>
          </a>
          {phones[1] && (
            <>
              <span className="hidden md:inline text-amber-800">|</span>
              <a href={`tel:${phones[1].number}`} className="flex items-center gap-1.5 hover:underline text-slate-950">
                <Phone size={13} className="text-slate-950 font-bold" />
                <span>{phones[1].label}: <strong className="font-bold">{phones[1].number}</strong></span>
              </a>
            </>
          )}
        </div>
