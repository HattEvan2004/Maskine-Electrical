...
export default function ServicesPanel({ onScrollToEstimator, onScrollToBooking }: ServicesPanelProps) {
  ...
  // Unique details for service showcase selection
    ...
    <section id="services" className="py-24 bg-slate-900 text-slate-100 border-t border-slate-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 flex flex-col items-center">
          ...
          {/* Filter Tabs */}
          <div className="flex flex-wrap gap-2 justify-center mt-8 bg-slate-950/80 p-1.5 rounded-xl border border-slate-800">
            {(['all', 'residential', 'commercial', 'emergency'] as const).map(category => (
              <button
                ...
                onClick={() => {
                  ...
                  const firstOfNew = SERVICES_DATA.find(s => {
                ...
                className={`px-4 py-2 text-xs sm:text-sm font-medium rounded-lg transition-all capitalize ${
        ...
        {/* Modular Grid Panel splits visual lists & detail panels */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          
          {/* Left panel: Compact Cards List */}
          ...
          {/* Right panel: Static Detail Spotlight */}
          {selectedService && (
