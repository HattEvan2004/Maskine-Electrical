      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 mb-12 text-left">
          ...
          <div className="md:col-span-5 flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-550 max-w-xs leading-relaxed">
              {site?.footerBlurb || 'Serving the Halifax Regional Municipality (HRM) with professional electrical installations, repairs, and responsive support. Available 24 Hours a day, 7 days a week!'}
            </p>

            <div className="flex items-center gap-3.5 mt-2">
              
                href={site?.instagramUrl || "https://www.instagram.com/maskine.electric/"}
                target="_blank"
              ...
              >
                ...
                href={site?.facebookUrl || "https://m.facebook.com/profile.php?id=100084234099043"}
                target="_blank"
                rel="noopener noreferrer"
                className="h-8 w-8 bg-slate-900 hover:bg-slate-800 text-amber-500 rounded-lg flex items-center justify-center transition-colors border border-slate-800 hover:border-slate-700"
                title="Facebook Page"
          ...
          <div className="md:col-span-3">
            ...
            <div className="flex flex-col gap-3 text-xs sm:text-sm">
              <button onClick={() => onNavigate('services')} className="hover:text-amber-400 text-left transition-colors cursor-pointer w-fit">Electrical Services</button>
              <button onClick={() => onNavigate('estimator')} className="hover:text-amber-400 text-left transition-colors cursor-pointer w-fit">Cost Calculator</button>
              <button onClick={() => onNavigate('area-checker')} className="hover:text-amber-400 text-left transition-colors cursor-pointer w-fit">HRM Service Areas</button>
              <button onClick={() => onNavigate('safety')} className="hover:text-amber-400 text-left transition-colors cursor-pointer w-fit">Reviews & Safety Tips</button>
              <button onClick={() => onNavigate('booking')} className="hover:text-amber-400 text-left transition-colors cursor-pointer w-fit">Appointment Dispatch</button>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-3 text-xs sm:text-sm">
              <div className="flex items-start gap-2.5">
                ...
                <span className="text-slate-300">
                  {site?.address || '136 Brook Street, Halifax, NS B3N 2A8'}
                </span>
              </div>
              ...
              <div className="flex items-start gap-2.5">
                <Mail size={16} className="text-amber-500 shrink-0 mt-0.5" />
                  ...
                  href={`mailto:${site?.email || 'maskine.electric@gmail.com'}`}
                  className="text-slate-300 hover:text-amber-400 transition-colors break-all"
                >
                  {site?.email || 'maskine.electric@gmail.com'}
                </a>
              </div>
              ...
              <div className="flex items-start gap-2.5 border-t border-slate-900 pt-3">
                ...
                <div className="flex flex-col text-slate-300 font-sans">
                  {(site?.phones || [{label:'Anthony',number:'902-802-5306'},{label:'Mich',number:'902-830-5921'}]).map((p: any, i: number) => (
                    <a key={i} href={`tel:${p.number}`} className="hover:text-amber-500 transition-colors mt-1">
        ...
        <div className="border-t border-slate-900 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-xs text-slate-600 text-center sm:text-left">
            © {currentYear} {site?.businessName || 'MaskinE Electric Ltd.'} All rights reserved. {site?.copyrightSuffix || 'Registered in Nova Scotia (HRM).'}
          </p>
